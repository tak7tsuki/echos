...
#include <sys/types.h>
#include <netdb.h>
#include <unistd.h>		// todo: check if neccesary
#include <thread>

#define PORT "12127"		// port to communicate
#define HOST "localhost" 	// todo: remove
...
void Client::send_msg_to_server(string host, string msg)
{
	...
	if (getaddrinfo(host.c_str(), PORT, &hints, &res)) {
